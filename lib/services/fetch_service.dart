...
class FetchService {
  ...
  Future<Venue> loadVenue(String venueUrl) async {
    final response =
        await http.get(Uri.parse('https://sperrstunde.org/$venueUrl'));
    if (response.statusCode == 200) {
      var document = html_parser.parse(response.body);
      var venueElement = document.querySelector('.single-venue');
      if (venueElement != null) {
        return Venue.fromElement(venueElement);
      } else {
